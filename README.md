...
V zadání není popsáno, co by mělo byt výstupem, takže to bude dejme tomu počet záznamů (tj. počet lidí) z TabA.
...
Funkce v SQL - vím o těchto funkcích : scalar function (S SQL), ITVF (S SQL) - tu jsem už dokonce kdysi použil :-) a MSTVF (T-SQL).
Z povahy dotazu soudím, že máte na mysli scalar function a ne ITVF a MSTFV, které vrací result set (pokud tedy výstupem není side effect) - stejně jako S PROC bez OUTPUT, která také vrací result set (pokud tedy výstupem není side effect), nepočítám-li status code.
...
- Scalar function může být pomalá, protože se `SELECT dbo.AddNumbers(5, 3)` vyhodnocuje pro každý řádek tabulky, a optimalizace dotazu je omezená. Já vím, existuje Scalar UDF Inlining, ale to nastuduji, až bude třeba.
...
```sql
DECLARE @output INT;
EXEC Sales.GetCustomerOrders @param = value, @output = @output OUTPUT;
```
...
**DML triggers:**
- `AFTER` (INSERT INTO, DELETE, UPDATE,...) - vyzkoušel jsem si kdysi.
...
## 9. Může být uložena SQL DB ve více nebo jen v jednom souboru?

SQL Server umožňuje zápis do více datových souborů najednou (na rozdíl od log files, kde se to zapisuje sekvenčně).
...
Jsou navíc features v T-SQL u stringových operací, datových funkcí. Ono učebnice T-SQL, kterou mám, vždy neuvádí rozdíl oproti S SQL, takže více příkladů rozdílů bych musel pracně hledat jinde.
...
O system catalogue views jsem teda vůbec nevěděl - může to mít zajímavé použití, jako např.:
...
3. COMMIT po změnách (to samé u branched kolegů)
...
7. Až se kód bude zdáti OK, budeme implementovati MERGE OF BRANCHES INTO MASTER

Ja vím, že v gitu je toho více, než jsem to popsal. Nicméně si myslím, že v případě spolupráce si vyrobím fake project, kde si to vyzkouším a naučím se to, myslím, že jeden den bude stačit, a když mi pomůžete (odměním se flaškou mé SQěLé slivovice), tak možná bude stačit jen hodina dvě ...
...
## 15. Co by mohla ukazovat chyba, kde bys ji teoreticky mohl očekávat?
...
Tento skript by měl být používán v kontextu, kdy jsme použili IDENTITY v CREATE TABLE ... (společně s PK nebo jiným UNIQUE constrains, abychom zajistili unikátnost), sekvenčnost bude zajištěna automaticky, např. `IDENTITY(1, 1)` - začneme jedničkou s krokem 1, nelze tuto sekvenčnost už měnit, pokud nemáme nastavenou volbu IDENTITY_INSERT jako ON.

```sql
SET IDENTITY_INSERT sometableWithIdentity ON -- nastaví volbu IDENTITY_INSERT jako ON (= zapne ji) v sloupci sometableWithIdentity 
INSERT INTO sometableWithIdentity 
   (IdentityColumn, col2, col3, ...) 
VALUES 
   (AnIdentityValue, col2value, col3value, ...) -- vloží do sloupce sometableWithIdentity dané hodnoty pro PK IdentityColumn
SET IDENTITY_INSERT sometableWithIdentity OFF -- nastaví volbu IDENTITY_INSERT jako OFF (= vypne ji) v sloupci sometableWithIdentity 
```
...
```sql
/****** Object:  StoredProcedure [dbo].[BKO_nastavParAkcUp]    Script Date: 10.07.2025 7:14:19 ******/
SET ANSI_NULLS ON
GO
```
...
```sql
-- ================================================================================
-- Autor:       jiri@hegmon.cz
-- Popis:       smaže propírovaní výpisů
-- Historie:    
--              2025 07 10
--              - vytvoření procedury
-- ================================================================================
...
AS
```
-- Vytvoříme novou nebo pozměníme existující stored procedure s parametrem @intIDHlavickaGoPay typu integer, na výstupu budu mít, funkcionálně řečeno, side effects (ekvivalent typu unit v F#)

```sql
BEGIN 
```
-- Začátek BEGIN-END bloku  
-- Každý SQL příkaz běží implicitně v samostatné transakci (autocommit),  
-- Není zde jedna společná transakce pro celou proceduru.  
...
```
-- Smaže dočasnou tabulku, pokud existuje (pozn. pro mne - # -> lokální temp tabulka -> pouze pro tuto session)
...
```
-- Do dočasné tabulky #tmp_IDs do sloupce IDUhradaVypis se přidají hodnoty z vyselektovaného result sets, tj. hodnoty z tabulky TabBKOGoPayPolozka při daných podmínkách. Mimochodem, není IDUhradaVypis Foreign_Key (soudě dle poznámky o "zrušení propírování")?
...
```
-- Do dočasné tabulky #tmp_IDs do sloupce IDUhradaVypis se přidají hodnoty z vyselektovaného result sets, tj. hodnoty ze sloupce IDUhradaPoplatkyVypis tabulky TabBKOGoPayHlavicka při daných podmínkách. Mimochodem, není IDUhradaPoplatkyVypis Foreign_Key (soudě dle poznámky o "zrušení propírování")?
...
```sql
    ...
    where IDHlavicka = @intIDHlavickaGoPay 
```
-- Update tabulky TabBKOGoPayPolozka tímto způsobem:  
-- nastavení těch hodnot ve sloupci IDUhradaVypis, kde platí daná podmínka, na NULL, přičemž sloupec IDUhradaVypis musí umožňovat NULL (nesmí mít NOT NULL constraint), jinak UPDATE skončí errorem. Pokud by byl PK, NULL by nebyl povolen vůbec.
...
```
-- Update tabulky TabBKOGoPayHlavicka tímto způsobem:  
...
```
-- Smaže hodnoty z tabulky TabBankVypisRUhrady, kde ID leží v množině hodnot mappované pomocí select IDUhradaVypis from #tmp_IDs (kdyby nad tím byl typ, tak ten by byl functor :-))
